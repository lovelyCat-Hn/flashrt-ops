...
# 环境本体（torch wheel / .so / tokenizer / 权重）不受影响——只换这 3 个 py 文件。
# 本目录的文件 = echo 机 2026-09-22 全链路验证过的版本。
#
...
if probe; then
    echo "✅ FlashRT 本地补丁已在（action_dim + norm_mode），无需处理"
    exit 0
fi

echo "→ 应用热修到 $DST ..."
for f in flash_rt/api.py flash_rt/frontends/torch/pi05_rtx.py flash_rt/core/utils/actions.py; do
    cp -v "$SRC/$f" "$DST/$f" || { echo "❌ 复制失败: $f"; exit 1; }
done

if probe; then
    echo "✅ 热修应用完成（3 文件，纯 Python，无需重装/重编）"
else
...
if git -C "$DST" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    if git -C "$DST" add -A && git -C "$DST" \
        ...
        commit -q -m "hotfix: action_dim 可配置 + norm_mode 语义/normalize_state（echo 机 bff59419 同步）
