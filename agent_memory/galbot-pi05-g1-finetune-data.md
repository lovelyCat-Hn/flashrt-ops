...
**state 侧夹爪单位 bug（2026-09-23 修复）**：`run_g1_execute/loop` 曾把 SDK 米直接喂 `normalize_state`（数据集 dim7/15 是 0~100%）——0.12m 被当 0.12% 算，模型恒看到「≈闭合」，夹爪物理全开也错。`run_g1_inference.py` 早有换算（read_state），execute/loop 补齐 `state_from_joints()`（clip 0~100）。**数据集实证：199 轨全部从夹爪 0%（闭合）起步**（episode 首帧 state[7]/[15] 全 0）→ `g1_pose_warmup.py` 加 `--grip`：闭合到 width_min 对齐训练起点，不闭合则首帧观测分布错位；warmup 的 read23 也按标定换算 0~100% 展示/判达标。修复后模型输出随真实夹爪状态变化（干跑 A/B 实证：state 修正前后夹爪维输出不同）。
