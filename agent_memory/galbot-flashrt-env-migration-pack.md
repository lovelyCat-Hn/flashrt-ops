...
metadata: 
  ...
  modified: 2026-09-21T09:10:23.934Z
---
...
**bundle v2 + 双轨管理（2026-09-21 定稿）**：**bundle 只装环境+资产**（miniforge3/FlashRT/cuda_warmup.py/pytorch/dist/models/.cache/flash_rt，**不含 scripts**，不压缩——safetensors 压不动）；**代码轨走 git**：`~/holy` 本身即仓库（.gitignore 圈住 FlashRT/models/pytorch，只跟踪 scripts+三份 MD+cuda_warmup.py），remote = `git@github.com:lovelyCat-Hn/flashrt-ops.git`（SSH 已通，2026-09-21 首推 master 完成）。换机 SOP：scp bundle → tar -xf → `cd ~/holy && git pull` → verify_deploy.sh。首版 v2 带过 scripts，无害，git 覆盖。新增 `holy/models/pi05_lerobot_base`（权重+openpi norm_stats）、`.cache/flash_rt/paligemma_tokenizer.model`。一键验收 `bash ~/holy/scripts/verify_deploy.sh`（⑤ graph 探测必须真 predict——graph 崩在首推理不在 load）。env 自包含含钉版 numpy==1.26.4+opencv-python-headless==4.10.0.84，目标机零 pip 零联网。9 条踩坑处置表在 `~/holy/DEPLOY.md`。
