...
- **红线：flash_pyrt311 里装任何带 numpy 依赖的包必须显式钉 `numpy==1.26.4`**（2026-09-21 装 opencv-python-headless 时被 pip 升到 2.4.6，torch 2.4 自编 wheel 是 numpy 1.x ABI，会炸）。兼容组合：`numpy==1.26.4 + opencv-python-headless==4.10.0.84`（OpenCV 5.x 要 numpy≥2，不能用）。2026-09-24 用户又装入 pandas 2.1.4+pyarrow 16.1.0（numpy 幸存，兼容）——环境与打包 bundle 出现分化，重打包时注意。
...
- **echo 机端到端推理已跑通（2026-09-21）**：加载 12s（arch=rtx_sm87 自动选中），predict 输出 (10,7)。还差两个随权重之外的文件：`paligemma_tokenizer.model`（4.26MB，GCS `storage.googleapis.com/big_vision/paligemma_tokenizer.model`，HF 的 google/paligemma 是 gated 仓拿不到；慢时用 `curl -C -` 续传）放 `~/.cache/flash_rt/`。脚本在 `~/holy/scripts/`，分子目录：`inference/`（load_pi05.py / load_pi05_int8.py 加载冒烟；**run_pi05_camera.py 真机相机推理入口**——SDK 取图+prompt 推理+动作打印，只读不执行，三层打点：纯推理/取图解码/端到端节拍+动作步率，自动对照控制频率窗口与 echo 基线）、`test/`（bench_pi05.py 延迟基准，用法 `python test/bench_pi05.py bf16|int8_enc|int8_full [n]`；graph_repro.py 抓图复现）、`eval/`（ab_compare_pi05.py 合成 A/B、ab_real_camera.py 真机相机 A/B），根上 verify_deploy.sh 为装机验收入口。**相机解码成本实测（2026-09-21）：3 路 SDK 抓图+cv2 软解+resize224 ≈ 20ms**，远小于推理耗时；真机 3 视角全 INT8 端到端节拍 ~248ms/轮（40.4 步/秒），50Hz 控制窗口 200ms 下**实测确认追不上（差 29ms）**，需 40Hz/2 视角/修 graph。
...
- **已解决（2026-09-24）：CUDA graph 抓图确定性段错误**——旧结论"与图内容/规模相关、最小复现通过"是误判：当日的 graph_repro.py 最小复现其实躲开了雷点。真根因=旧式 `cudaGraphInstantiate` 符号在该机 tegra 驱动（L4T r35.6 iGPU）对**任意图**必段错误（单节点 memset 也崩，与 libcudart 版本无关）；`cudaGraphInstantiateWithFlags` 入口正常（torch 原生能过正因如此）。修复=FlashRT cuda_graph.py 改走 WithFlags（提交 4822d755，已入 hotfix_flashrt），细节见 [[galbot-g1-cuda-graph-instantiate-fix]]。脚本 monkeypatch/`PI05_NO_GRAPH` 开关保留作回退，默认已翻转开图；bench 实测 graph 收益 ~3%（GPU-bound）。
