...
**节拍化续案（2026-09-24 v4）**：轨迹锁定的"步末到点"本身也会造成 1Hz 停走（一顿一顿）。正解=**前视 2 窗口 + 半程配速**：目标=参考位前视 6 帧，速度=导程÷(2×窗口)——每窗只走一半导程，目标永远差一半路程，臂恒在途中被下一条指令滑行重定向，永不到点。两个反面教材：① 配速按全导程=窗口末恰好到点，前视形同虚设；② 计时窗口必须从**发令前**起算（推理重叠在内），t_s 取在推理后 = 推理串行在窗外，每步干等 ~0.5s（v3 实测 293s/198 步 vs 窗口 0.95s 抓的虫）。SDK 滑行重定向承担衔接平滑，等价于外部方案（~/pi05_smooth_execution_plan.md）的融合窗口，其 1ms RT 层方案在 SDK 黑盒下不可实现。
