...
metadata: 
  ...
  modified: 2026-09-22T09:45:47.076Z
---
...
- **How to apply**：拿到厂商文档或台架条件前，勿在真机跑 `g1_traj_probe.py` 或 `run_g1_loop.py --chunk-mode traj`。平滑运动需求走 `--chunk-mode track`（追踪式单步）+ 提高 `--speed`（如 0.6）+ `--settle-frac 0.5` 的路线，每步位移钳位与漂移护栏不变。相关：[[galbot-pi05-g1-finetune-data]]
