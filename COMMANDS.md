...
- config 内置 2026-09-23 真机 5/5 全绿组合 + 2026-09-24 v4 半程配速（与回放
  ...
  ——速度=导程÷(2×窗口)，臂恒在途永不到点、指令边界滑行换向（治切换抖动；
  旧 settle-frac/switch-dist 追踪式已不用，保留兼容）
- 漂移护栏：任一关节偏离起始位 >3.0 rad 自动停（`--max-excursion` 调；
  按数据集 199 轨合法抓取包络 max 2.785 重标，旧 0.25 真任务 100% 误触）
- rounds 默认 200：数据集单次抓取 ≈198 控制步（594 帧÷3）；v4 半程配速下
  每轮净行程 ~10-16 mrad，60 轮只走 ~1/3（2026-09-24 首跑 987 mrad 中断实证）
- 推理在执行期间后台完成，p50 ~350 ms（bf16 档）、重规划 ~2 Hz 为正常水位

**看什么**：每轮遥测分位数；导程残差 mrad（追踪式 v4 半程配速，>0 属预期
...
- **观测全部来自数据集**（3 路相机按时间戳取帧 + 23 维 state 归一化喂模型），
  ...
  慢放零语义损失（模型看不见机器人）
- **节拍执行（2026-09-24 v4，治"一顿一顿"）**：每控制步 1 条臂目标=数据集
  参考位【前视 6 帧】，速度=导程÷(2×`[replay].pace`)半程配速（自适应
  0.02~`[replay].speed`）——每窗只走一半导程，臂恒在途永不到点，被下一条
  指令滑行重定向；窗口自发令前起算、推理重叠在内（v3 两个停走根因的修复：
  窗口计时 bug 让推理串行在窗外每步干等 ~0.5s；配速按全导程恰好到点）。
  夹爪用上一 chunk 末行（滞后 1 数据集帧）。到位确认保留（超时 catch_timeout
  警告推进）。汇总看"前视导程 gap"恒 >0 与"残差"两列
