...
## 附录：CUDA graph 本机实测——修复有效但反而更慢，定档 eager（2026-09-24，源机）

背景：部署机线破案 r35.6 旧式 `cudaGraphInstantiate` 必段错误，修复=WithFlags
（hotfix_flashrt/，FlashRT 快照 7a445131）。源机打上热修后首次 graph 验收
（`PI05_NO_GRAPH=0 run_g1_inference`，bf16/3 视角/fixed 模式）：

| 模式 | 纯推理 p50（10 轮） | 对 333ms 控制窗 |
|---|---|---|
| eager（历史基线） | 304.0 ms | ✅ 余量 29ms |
| graph | 391.5（min 385.3 / max 397.2） | ❌ 超窗 ~60ms（**+29%**） |

- 修复有效性 ✅：不再段错误，autotune 完整、输出 16 维量级正确（臂 ±0.04 rad
  平滑增量、夹爪 1.2%/0% ≈ 闭合态）
- 性能与部署机（graph 快 ~3%）**相反**：本机 graph 恒慢 ~88ms 且 10 轮极稳
