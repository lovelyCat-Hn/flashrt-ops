...
用法:
  ...
  ~/miniforge3/envs/flash_pyrt311/bin/python \
      ...
      [--delta-max 0.05] [--force-state-dim] [--prompt "..."]
...
GRIP_DIMS = (7, 15)   # chunk 里的夹爪维（dim7 右 / dim15 左；本脚本不下发）
...
print(f"夹爪维首步值 {np.round(g, 3).tolist()}（0~100 语义，本脚本不下发夹爪）")
...
input(f"\n⚠ 将按 ≤{args.speed} rad/s 真实驱动双臂 {n_steps} 步。急停就绪后回车开始，"
      f"Ctrl-C 中止...")
